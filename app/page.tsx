...
function HomeContent() {
  ...
  return (
    // overflow-x-hidden strictly added here to prevent right side blank space
    <div className="font-sans flex flex-col bg-[#f4f5f7] min-h-screen w-full overflow-x-hidden">
      ...
      <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row px-4 gap-4 md:gap-6 flex-grow pb-10 w-full">
        ...
        <main className="flex-1 w-full min-w-0">
          ...
          {/* Responsive Stores Carousel with Arrows */}
          <div className="relative mb-6 md:mb-8">
            ...
            >
              {supermarkets.map((store) => (
                <div key={store.id} className="flex flex-col items-center min-w-[60px] md:min-w-[70px] snap-start cursor-pointer group shrink-0">
                  <div className="w-12 h-12 md:w-14 md:h-14 rounded-full border border-gray-200 overflow-hidden group-hover:border-green-500 transition-all shadow-sm p-0.5">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end mb-4 md:mb-5 border-b border-gray-200 pb-3 gap-3 w-full">
            ...
            <div className="flex gap-2">
              <button className="bg-green-600 text-white px-4 md:px-5 py-1.5 rounded-md text-xs font-bold shadow-sm hover:bg-green-700 transition-colors">Top Pick</button>
              <button className="bg-white text-gray-700 px-4 md:px-5 py-1.5 rounded-md text-xs font-bold border border-gray-300 hover:bg-gray-50 transition-colors">Latest</button>
            </div>
          </div>

          {/* Responsive Flyer Carousel with Arrows */}
          <div className="relative mb-8">
            ...
            >
              {flyers.map((flyer) => (
                // Mobile: 2 items (50% width minus gap). Desktop: 3 items (33.3% width minus gap). Smaller card layout.
                <div key={flyer.id} className="w-[calc(50%-0.375rem)] md:w-[calc(33.333%-0.66rem)] snap-center bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer group flex flex-col relative shrink-0">
